...
public class RouterAgent implements Agent {
    ...
    public static final String AGENT_DESCRIPTIONS_PLACEHOLDER = "AGENT_DESCRIPTIONS";
    ...
    /**
     * 构建路由 ChatClient：从 Nacos 加载 prompt 模板，通过 PromptTemplate 替换 Agent 描述占位符。
     */
    private ChatClient buildRouterChatClient() {
        String basePrompt = promptLoader.load(AgentType.ROUTE);
        String agentDescriptions = agentMap.values().stream()
                .map(this::formatAgentDescription)
                .collect(Collectors.joining("\n"));
        PromptTemplate promptTemplate = new PromptTemplate(basePrompt);
        String systemPrompt = promptTemplate.render(Map.of(AGENT_DESCRIPTIONS_PLACEHOLDER, agentDescriptions));
